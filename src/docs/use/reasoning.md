...
如果未指定`max_tokens`，我们会默认赋值`Claude`允许的最大 tokens 传入。 例如`claude-3-7`的最大 tokens 为`128000`。

### Claude 3.7 模型开启 thinking 

在请求时，将模型名称后面添加 `#thinking` 可以开启 thinking 模式。 我们会自动将 `max_tokens` 的 80%作为`budget_tokens`传入给 Claude。
...
`reasoning.max_tokens` 参数可以设置为具体的数值，表示推理最大 tokens，目前特指 budget\_tokens。
